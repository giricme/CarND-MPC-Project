...
// variables in a single vector. These constants help know
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    // 4 * 10 + 2 * 9. Also N timesteps == N - 1 actuations
    ...
    // `solution.x[i]`. {...} is shorthand for creating a vector, so auto x1 = {1.0,2.0}
    // creates a 2 element double vector.
    std::vector<double> retval = {solution.x[delta_start], solution.x[a_start], (double)N};
    
    // extract mpc x & y values
    for (int i=0; i <N; i++) {
        retval.push_back(solution.x[i + x_start + 1]);
    }
    
    for (int i=0; i <N; i++) {
        retval.push_back(solution.x[i + y_start + 1]);
    }
    return retval;
}
